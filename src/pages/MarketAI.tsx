...
const explainers = [
  ...
  { question: "How should I read a market rate?", answer: "Compare the price and percentage change with the quote's timestamp, volume and relevant company announcement. A delayed quote is for learning, not for placing an order." },
];
function MarketRatesWidget() {
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({ colorTheme: "dark", dateRange: "1D", locale: "en", isTransparent: true, showFloatingTooltip: true, tabs: [{ title: "India", symbols: [{ s: "NSE:NIFTY", d: "NIFTY 50" }, { s: "NSE:BANKNIFTY", d: "NIFTY Bank" }, { s: "NSE:RELIANCE", d: "Reliance" }, { s: "NSE:TCS", d: "TCS" }, { s: "NSE:HDFCBANK", d: "HDFC Bank" }] }], width: "100%", height: 430, showSymbolLogo: true, showChart: true });
    container.current.appendChild(script);
    return () => { if (container.current) container.current.innerHTML = ""; };
  }, []);
  return <div className="mai-rates-widget" ref={container} aria-label="TradingView Indian market quotes" />;
}
export default function MarketAI() {
  ...
  return <div className="mai-page"><Navbar /><main className="mai-container">
    ...
    <div className="mai-ticker"><span>MARKET PULSE</span><div>Indian market education &nbsp; ✦ &nbsp; Quotes may be delayed &nbsp; ✦ &nbsp; Verify execution prices on your trading platform</div></div>
    <section className="mai-section" id="rates"><div className="mai-heading"><div><span className="mai-kicker">INDIAN MARKET</span><h2>Market rates</h2></div><span className="mai-badge">TradingView · delayed data</span></div><p>Explore Indian index and share prices for education. Exchange data in this free widget may be delayed; check the widget's timestamp and your trading platform before any trade.</p><MarketRatesWidget /><small>Rates shown by TradingView. This is an external widget; availability and delay depend on its data provider.</small></section>
    ...
    <p className="mai-disclaimer">Market information and education only. Embedded rates can be delayed and are not execution prices. Master Trader does not publish stock-specific Buy, Sell, Stop Loss or Target recommendations on this page. Verify dates and market information with the original exchange or publisher.</p>
  </main><Footer /></div>;
}
